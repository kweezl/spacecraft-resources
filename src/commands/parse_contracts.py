...
from src.lib import config, contracts
...
def run(data: str | None = None, out: str | None = None, dry_run: bool = False) -> int:
    ...
    out_path = resolve(out, config.PARSE_CONTRACTS_OUT)
    try:
        result = contracts.parse_contracts(data_path)
    ...
    typer.echo(f"{action} {result['count']} contracts ({result['skipped']} skipped)")
    if not dry_run:
...
def command(
    ...
    out: str | None = typer.Option(None, "--out", help="Output JSON path. Env: SC_PARSE_CONTRACTS_OUT."),
