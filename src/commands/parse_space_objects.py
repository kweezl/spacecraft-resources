...
from src.lib import config, space_objects
...
def run(data: str | None = None, out: str | None = None, dry_run: bool = False) -> int:
    ...
    out_path = resolve(out, config.PARSE_SPACE_OBJECTS_OUT)
    try:
        result = space_objects.parse_space_objects(data_path)
    ...
    typer.echo(f"{action} {result['count']} space objects ({result['skipped']} skipped)")
    if not dry_run:
...
def command(
    ...
    out: str | None = typer.Option(None, "--out", help="Output JSON path. Env: SC_PARSE_SPACE_OBJECTS_OUT."),
