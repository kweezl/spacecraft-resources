...
from src.lib import config, factions
...
def run(data: str | None = None, out: str | None = None, dry_run: bool = False) -> int:
    ...
    out_path = resolve(out, config.PARSE_FACTIONS_OUT)
    try:
        result = factions.parse_factions(data_path)
    ...
    typer.echo(f"{action} {result['count']} factions ({result['skipped']} skipped)")
    if not dry_run:
...
def command(
    ...
    out: str | None = typer.Option(None, "--out", help="Output JSON path. Env: SC_PARSE_FACTIONS_OUT."),
