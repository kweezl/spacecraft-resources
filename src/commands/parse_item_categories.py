...
from src.lib import config, item_categories
...
def run(data: str | None = None, out: str | None = None, dry_run: bool = False) -> int:
    ...
    out_path = resolve(out, config.PARSE_ITEM_CATEGORIES_OUT)
    try:
        result = item_categories.parse_item_categories(data_path)
    ...
    typer.echo(f"{action} {result['count']} categories ({result['skipped']} skipped)")
    if not dry_run:
...
def command(
    ...
    out: str | None = typer.Option(None, "--out", help="Output JSON path. Env: SC_PARSE_ITEM_CATEGORIES_OUT."),
