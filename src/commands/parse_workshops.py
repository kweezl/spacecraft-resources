...
from src.lib import config, workshops
...
def run(data: str | None = None, out: str | None = None, dry_run: bool = False) -> int:
    ...
    out_path = resolve(out, config.PARSE_WORKSHOPS_OUT)
    try:
        result = workshops.parse_workshops(data_path)
    ...
    typer.echo(f"{action} {result['count']} workshops ({result['skipped']} skipped)")
    if not dry_run:
...
def command(
    ...
    out: str | None = typer.Option(None, "--out", help="Output JSON path. Env: SC_PARSE_WORKSHOPS_OUT."),
