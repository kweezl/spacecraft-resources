...
from src.commands import parse_craft as cmd

SAMPLE = {"source": "x", "sheet": "craft", "count": 0, "skipped": 0, "recipes": {}}


class ParseCraftCommandTests(unittest.TestCase):
    def test_run_resolves_default_data(self):
        with mock.patch.dict(os.environ, {}, clear=True):
            with mock.patch("src.lib.craft.parse_craft", return_value=SAMPLE) as m:
    ...
    def test_dry_run_writes_nothing(self):
        with mock.patch.dict(os.environ, {}, clear=True):
            with mock.patch("src.lib.craft.parse_craft", return_value=SAMPLE):
                with tempfile.TemporaryDirectory() as tmp:
                    out = Path(tmp) / "craft.json"
    ...
    def test_writes_json_to_resolved_out(self):
        with mock.patch.dict(os.environ, {}, clear=True):
            with mock.patch("src.lib.craft.parse_craft", return_value=SAMPLE):
                with tempfile.TemporaryDirectory() as tmp:
                    out = Path(tmp) / "sub" / "craft.json"
                    cmd.run(out=str(out))
                    self.assertTrue(out.exists())
                    self.assertEqual(
                        json.loads(out.read_text(encoding="utf-8"))["sheet"], "craft"
                    )

    def test_returns_1_on_error(self):
        with mock.patch.dict(os.environ, {}, clear=True):
            with mock.patch("src.lib.craft.parse_craft", side_effect=ValueError("boom")):
                code = cmd.run(out=os.devnull, dry_run=True)
