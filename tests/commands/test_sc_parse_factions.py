...
from src.commands import parse_factions as cmd

SAMPLE = {"source": "x", "sheet": "faction", "count": 0, "skipped": 0, "factions": {}}


class ParseFactionsCommandTests(unittest.TestCase):
    def test_run_resolves_default_data(self):
        with mock.patch.dict(os.environ, {}, clear=True):
            with mock.patch("src.lib.factions.parse_factions", return_value=SAMPLE) as m:
    ...
    def test_dry_run_writes_nothing(self):
        with mock.patch.dict(os.environ, {}, clear=True):
            with mock.patch("src.lib.factions.parse_factions", return_value=SAMPLE):
                with tempfile.TemporaryDirectory() as tmp:
                    out = Path(tmp) / "factions.json"
    ...
    def test_writes_json_to_resolved_out(self):
        with mock.patch.dict(os.environ, {}, clear=True):
            with mock.patch("src.lib.factions.parse_factions", return_value=SAMPLE):
                with tempfile.TemporaryDirectory() as tmp:
                    out = Path(tmp) / "sub" / "factions.json"
                    cmd.run(out=str(out))
                    self.assertTrue(out.exists())
                    self.assertEqual(
                        json.loads(out.read_text(encoding="utf-8"))["sheet"], "faction")

    def test_returns_1_on_error(self):
        with mock.patch.dict(os.environ, {}, clear=True):
            with mock.patch("src.lib.factions.parse_factions", side_effect=ValueError("boom")):
                code = cmd.run(out=os.devnull, dry_run=True)
