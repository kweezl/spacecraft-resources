...
| `icons/<name>.webp` | filename | The pre-cropped, recoloured 64px icons (lossless WebP). |

```
items.json                 icons_manifest.json              aliases.json
  "IronOre": {               "IronOre": {                     "icons": {
    "id": "IronOre",           "id": "IronOre",                 "IronOre": "IronOre.webp",
    "icon": {                  "output": ".../IronOre.webp",    "Access_Corpo2": "Access_Corpo1.webp",
...
Icons ship as **lossless WebP** (smaller than PNG, ideal for web/Discord). They
are generated from each item's own `icon` coordinates, so a file is normally
named after its item id (`IronOre` → `IronOre.webp`). Visually identical icons
are then deduplicated and **share one file**: e.g. `Access_Corpo2` resolves to
`Access_Corpo1.webp`. That is why you must resolve the filename through the map
rather than assuming `<id>.webp` always exists. (The icon set can also be
produced as PNG via `generate-icons --format png`.)
...
icon_path = entry["output"] if entry else None   # e.g. "generated/icons/IronOre.webp"
```

## Caveats

1. **Do not assume `icons/<id>.webp` exists.** Visually identical icons are
   deduplicated: only ~260 *canonical* files are on disk for 571 items. Many ids
   resolve to a **shared** file (e.g. `Access_Corpo2` → `Access_Corpo1.webp`).
...
2. **A few items have no icon at all.** Icons are generated for every item that
   ...
   markers — have no `icon` block and therefore no file. A lookup returns nothing
